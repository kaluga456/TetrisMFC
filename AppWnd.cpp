...
bool CMoveKeyTimer::OnKeyDown(UINT key)
{
	...
	ASSERT(0 == Timer);
	Key = key;
	Timer = ::SetTimer(Parent, MOVE_TIMER_ID, Options.KeyboardSpeed, nullptr);
	return true;
}
...
CAppWnd::CAppWnd() : CFrameWnd()
{
	...
	//init timers
	TimeTimerId = TIME_TIMER_ID;
	SpeedTimerId = SPEED_TIMER_ID;
	MoveKeyTimer.SetParent(m_hWnd);

	//create controls
	GameFieldView = new CGameFieldView(this, 
		APPWND_SPACING, 
		APPWND_SPACING, &GameField);
	NextShapeView = new CNextShapeView(this, 
		APPWND_SPACING + GAME_FIELD_VIEW_WIDTH + APPWND_SPACING,
		APPWND_SPACING );
	ScoreLabel = new CTextView(this, 
		APPWND_SPACING + GAME_FIELD_VIEW_WIDTH + APPWND_SPACING, 
		NEXT_SHAPE_VIEW_HEIGHT + 3*APPWND_SPACING, 
		TEXT_VIEW_WIDTH, TEXT_VIEW_HEIGHT, L"SCORE");
	ScoreView = new CTextView(this, 
		APPWND_SPACING + GAME_FIELD_VIEW_WIDTH + APPWND_SPACING, 
		NEXT_SHAPE_VIEW_HEIGHT + 3*APPWND_SPACING + TEXT_VIEW_HEIGHT, 
		TEXT_VIEW_WIDTH, TEXT_VIEW_HEIGHT, L"");
	LinesLabel = new CTextView(this, 
		APPWND_SPACING + GAME_FIELD_VIEW_WIDTH + APPWND_SPACING, 
		NEXT_SHAPE_VIEW_HEIGHT + 5*APPWND_SPACING + 2*TEXT_VIEW_HEIGHT, 
		TEXT_VIEW_WIDTH, TEXT_VIEW_HEIGHT, L"LINES");
	LinesView = new CTextView(this, 
		APPWND_SPACING + GAME_FIELD_VIEW_WIDTH + APPWND_SPACING, 
		NEXT_SHAPE_VIEW_HEIGHT + 5*APPWND_SPACING + 3*TEXT_VIEW_HEIGHT, 
		TEXT_VIEW_WIDTH, TEXT_VIEW_HEIGHT, L"");
	BonusLabel = new CTextView(this, 
		APPWND_SPACING + GAME_FIELD_VIEW_WIDTH + APPWND_SPACING, 
		NEXT_SHAPE_VIEW_HEIGHT + 7*APPWND_SPACING + 4*TEXT_VIEW_HEIGHT, 
		TEXT_VIEW_WIDTH, TEXT_VIEW_HEIGHT, L"BONUS");
	BonusView = new CTextView(this, 
		APPWND_SPACING + GAME_FIELD_VIEW_WIDTH + APPWND_SPACING, 
		NEXT_SHAPE_VIEW_HEIGHT + 7*APPWND_SPACING + 5*TEXT_VIEW_HEIGHT, 
		TEXT_VIEW_WIDTH, TEXT_VIEW_HEIGHT, L"");
	TimeLabel = new CTextView(this, 
		APPWND_SPACING + GAME_FIELD_VIEW_WIDTH + APPWND_SPACING, 
		NEXT_SHAPE_VIEW_HEIGHT + 9*APPWND_SPACING + 6*TEXT_VIEW_HEIGHT, 
		TEXT_VIEW_WIDTH, TEXT_VIEW_HEIGHT, L"TIME");
	TimeView = new CTextView(this, 
		APPWND_SPACING + GAME_FIELD_VIEW_WIDTH + APPWND_SPACING, 
		NEXT_SHAPE_VIEW_HEIGHT + 9*APPWND_SPACING + 7*TEXT_VIEW_HEIGHT, 
		TEXT_VIEW_WIDTH, TEXT_VIEW_HEIGHT, L"");
}
CAppWnd::~CAppWnd()
{
...
{
	NextShapeView->SetShape(GameField.NextShape);
}
...
void CAppWnd::OnLinesDelete(int lines_count)
{
	...
	ScoreView->SetText(GameField.GetTotalScore());
...
void CAppWnd::OnPaint()
{
	...
	ScoreView->OnPaint(&dc);
	ScoreLabel->OnPaint(&dc);
	LinesView->OnPaint(&dc);
	LinesLabel->OnPaint(&dc);
	BonusView->OnPaint(&dc);
	BonusLabel->OnPaint(&dc);
...
void CAppWnd::OnNewGame()
{
	...
	TimeTimerId = SetTimer(TimeTimerId, 1000, NULL);
	ASSERT(TimeTimerId);
	Speed = 1000; //1 sec
	SpeedTimerId = SetTimer(SpeedTimerId, Speed, NULL);
	ASSERT(SpeedTimerId);
	GameTimer.OnNewGame();
	...
	ScoreView->SetText(L"0");
	LinesView->SetText(L"0");
	BonusView->SetText(L"+0%");
}
...
{
	if(MOVE_TIMER_ID == nIDEvent)
	{
		if (GS_RUNNING == GameState)
			GameField.OnShapeMove(MoveKeyTimer.GetMoveType());
	}
	else if(nIDEvent == TimeTimerId)
	{
		if(GameState == GS_RUNNING)		//increase game speed
		{
			Speed -= ACCELERATION;
			if(Speed > 0)
				SpeedTimerId = SetTimer(SpeedTimerId, Speed, NULL);
			ASSERT(SpeedTimerId);
			TimeView->SetText(GameTimer.GetGameSpanString());
		}
	}
	else if(nIDEvent == SpeedTimerId)
	{
		if(GameState == GS_RUNNING)
			GameField.OnTimer();
	}
